...
*/
static int
handle_conserver_death_notification(struct proc_notification *notification)
{
    dprintf(COLOUR_Y "## Console serv Handling death notification...\n" COLOUR_RESET);
    dprintf("     Label: PROCSERV_NOTIFY_DEATH\n");
    dprintf("     deathID: %d\n", notification->arg[0]);

    /* Find the client and queue it for deletion. */
    int error = client_queue_delete_deathID(&conServCommon->clientTable, notification->arg[0]);

    if (error) {
        ROS_ERROR("Unknown deathID. Console server book-keeping error.");
        assert(!"Console Server book-keeping bug.");
...
{
    if ((m->badge & CONSERV_ASYNC_BADGE_MASK) == 0) {
        return DISPATCH_PASS;
    }
    if ((m->badge & CONSERV_ASYNC_NOTIFY_BADGE) == 0) {
        return DISPATCH_PASS;
    }
    ...
    srv_common_notify_handler_callbacks_t cb = {
        ...
        .handle_server_death_notification = handle_conserver_death_notification
    };

    return srv_dispatch_notification(conServCommon, cb);
}
