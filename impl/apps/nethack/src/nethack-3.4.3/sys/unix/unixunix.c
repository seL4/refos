...
#endif
#if defined(__linux__) && REFOS_LINUX /* Don't define linux in RefOS by default */
...
getlock()
{
	...
	if(locknum) {
        ...
        goto gotlock; // No file creation support in RefOS.
        #if 0
		fq_lock = fqname(lock, LEVELPREFIX, 0);
		if((fd = open(fq_lock, 0)) == -1) {
			if(errno == ENOENT) goto gotlock;    /* no such file */
